...
const HotelBillInvoiceInHistory = () => {
  ...
  const subTotalKeyLost = useMemo(
    () =>
      invoiceData?.keysDataList?.filter((item) => item?.isKeyLost)?.length || 0,
    [invoiceData]
  );
  ...
  const subTotalExpense = useMemo(
    () =>
      parseFloat(
        invoiceData?.transactionDetails
  ...
  const subTotalExtraItemsExpense = useMemo(() => {
    const sum =
      invoiceData?.extraItemsList?.reduce(
  ...
  const subTotalLaundryExpense = useMemo(() => {
    const sum =
      invoiceData?.laundryDataList?.reduce(
        (sum, item) => sum + (item?.totalPrice || 0),
        0
      ) || 0;
  ...
  const subTotalFoodExpense = useMemo(() => {
    const sum =
      invoiceData?.foodDataList?.reduce(
  ...
  const subTotalRoomCharges = useMemo(() => {
    const sum =
      ...
      //   0
      // ) || 0;
      parseFloat(invoiceData?.gstPrice + invoiceData?.totalRoomCharge);
  ...
  const uniqueFoodItems = useMemo(() => {
    const foodList = Boolean(invoiceData?.foodDataList?.length)
      ? invoiceData?.foodDataList
      : [];
    const foodItemsMap = {};
  ...
  const uniqueLaundryItems = useMemo(() => {
    const laundryList = Array.isArray(invoiceData?.laundryDataList)
      ? invoiceData.laundryDataList
      : [];
        ...
        >
          <Grid container size={12}>
            <Grid size={{ xs: 12 }}>
              ...
              >
                {Boolean(invoiceData?.foodDataList?.length) && (
                  <Grid size={6}>
                    {Boolean(invoiceData?.foodDataList?.length) && (
                      <Box
                        sx={{
                      ...
                      >
                        ...
                        <CustomFoodListTableContainerForInvoice
                          foodListTableHeaders={foodListTableHeaders}
                          foodListTableData={invoiceData?.foodDataList}
                          isForCheckOut={true}
                ...
                {/* --- */}
                {Boolean(
                  Boolean(invoiceData?.bookingMapData?.length) ||
                    Boolean(invoiceData?.laundryDataList?.length) ||
                    Boolean(invoiceData?.extraItemsList?.length)
                ) && (
                  <Grid size={12}>
                    {Boolean(invoiceData?.bookingMapData?.length) && (
                      <Box
                    ...
                    {Boolean(invoiceData?.laundryDataList?.length) && false && (
                      <Box
                        sx={{
                          width: "100%",
                        }}
...
export default HotelBillInvoiceInHistory;
